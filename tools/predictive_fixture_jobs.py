...
def main():
    ...
    expects = {
        "mechanical": {"generator": ["athena"], "role_bits": [2, 65, 16],
                       "source": ["perimeters", "fills", "skirt_template", "brim_template"]},
        "thin_wall": {"generator": ["arachne"], "source": ["perimeters"], "emitted_roles": ["External perimeter"]},
        "bridge_support": {"role_bits": [706, 8, 72], "source": ["support"],
                           "emitted_roles": ["Bridge infill", "Support material", "Support interface"]},
        "multi_object": {"object": [0, 1], "minimum_object_labels": 4},
        "serpentine": {"role_bits": [4161], "emitted_roles": ["Serpentine"]},
        "interlocking": {"role_bits": [2049], "emitted_roles": ["Interlocking perimeter"]},
        "multi_material": {"configured_tool": [0, 1], "material": ["PLA", "PETG"],
                           "emitted_tools": [0, 1], "emitted_roles": ["Wipe tower"]},
    }
...
""", encoding="utf-8", newline="\n")
    ...
    for name, objects in geometry.items():
        amf(name, objects)
        (ROOT / f"{name}.ini").write_text(overrides[name], encoding="utf-8", newline="\n")
        jobs.append({"name": name, "models": [f"{name}.amf"], "profiles": ["base.ini", f"{name}.ini"], "expect": expects[name]})
    manifest = {"qualification_record": "../production/evidence.json", "volatile_comment_prefixes": ["; generated by preFlight 1.2.0 on "], "jobs": jobs}
