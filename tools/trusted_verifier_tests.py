...
EVIDENCE = {"starting_commit": "9228575df549161ff77f6b103580aa16c2f2d91a",
...
SCHEMAS = {name: jsonschema.Draft202012Validator(json.loads((ROOT / "docs/predictive-slicer/schema" / f"{name}.schema.json").read_text()))
           for name in ("verifier-machine", "verification-manifest", "verifier-report")}
...
class VerifierTests(unittest.TestCase):
    def run_verify(self, program, fingerprint=FINGERPRINT, binding=None):
        if binding is None:
            binding = manifest(program, fingerprint)
        with tempfile.TemporaryDirectory() as temp:
            folder = Path(temp)
            files = [folder / n for n in ("program.gcode", "machine.json", "manifest.json")]
            payloads = [program, fingerprint, encoded(binding) if not isinstance(binding, bytes) else binding]
            ...
            self.assertEqual(report["input_sha256"], dict(zip(("program", "fingerprint", "manifest"),
    ...
    def test_accepted_goldens(self):
        ...
        for name, program in GOLDENS.items():
            with self.subTest(name=name):
                ...
                self.assert_scanned(report, program)
                self.assertEqual(raw, self.run_verify(program, binding=binding)[1])
